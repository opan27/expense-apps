...
exports.getAll = async (req, res) => {
  ...
  const mapped = rows.map(r => {
    const dynamicRemaining = computeRemainingMonths(r.start_date, r.total_months);
    return {
      ...r,
...
exports.getUpcomingDue = async (req, res) => {
  try {
    const userId = req.user.userId;
    const daysAhead = 10;
    const rows = await installmentService.getUpcomingInstallments(userId, daysAhead);
    res.json(rows);
  } catch (err) {
...
exports.create = async (req, res) => {
  ...
  if (!total_months || isNaN(total_months)) {
    return res.status(400).json({ error: 'total_months wajib diisi dan harus angka.' });
  }
  if (!monthly_payment) {
